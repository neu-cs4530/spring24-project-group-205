...
describe('ScavengerHuntArea', () => {
  ...
  describe('EndGame command', () => {
    describe('no game in progress', () => {
      it('should throw an error', () => {
        expect(() =>
          gameArea.handleCommand({ type: 'EndGame', gameID: nanoid() }, player1),
        ).toThrowError(GAME_NOT_IN_PROGRESS_MESSAGE);
        expect(interactableUpdateSpy).not.toHaveBeenCalled();
      });
    });
    describe('game in progress', () => {
      ...
      describe('should call end on the game and call _emitAreaChanged', () => {
        it('when only player ends the game', () => {
          const { gameID } = gameArea.handleCommand(
            { type: 'JoinTimedGame', themepack: 'fruit' },
            player1,
          );
          ...
          expect(interactableUpdateSpy).toHaveBeenCalledTimes(1);
          const endSpy = jest.spyOn(gameTimed, 'endGame');
          gameArea.handleCommand({ type: 'EndGame', gameID }, player1);
          expect(endSpy).toHaveBeenCalledWith(player1);
          expect(interactableUpdateSpy).toHaveBeenCalledTimes(2);
        });
      });
    });
  ...
  describe('RequestHint command', () => {
    describe('no game in progress', () => {
      it('should throw an error', () => {
        expect(() =>
          gameArea.handleCommand({ type: 'RequestHint', gameID: nanoid() }, player1),
        ).toThrowError(GAME_NOT_IN_PROGRESS_MESSAGE);
        expect(interactableUpdateSpy).not.toHaveBeenCalled();
      });
    });
    describe('game in progress', () => {
      ...
      describe('should call requestHint on the game and call _emitAreaChanged', () => {
        it('when only player requests hint', () => {
          const { gameID } = gameArea.handleCommand(
            { type: 'JoinTimedGame', themepack: 'fruit' },
            player1,
          );
          ...
          expect(interactableUpdateSpy).toHaveBeenCalledTimes(1);
          const requestHintSpy = jest.spyOn(gameTimed, 'requestHint');
          gameArea.handleCommand({ type: 'RequestHint', gameID }, player1);
